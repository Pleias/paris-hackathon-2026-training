...
def setup_attention_backend(device: str, master: bool, require_fa4: bool) -> str:
    """Detect attention backend and return the backend string ("fa4" or "sdpa").

    The returned string is passed into the model config as ``attn_backend``
    so that ``CausalSelfAttention`` can choose the right kernel at init time.
    """
    ...
    if fa4_available:
        backend = "fa4"
        if master:
            print(f"[attn] flash_attn {flash_attn_version} detected — using FA4 native API for attention.")
    else:
        backend = "sdpa"
        ...
        if master:
            print("[attn] flash_attn>=4 not detected — using PyTorch SDPA flash kernel for attention.")
...
def main():
    ...
    attn_backend = setup_attention_backend(device=device, master=master, require_fa4=args.require_fa4)
    ...
    while step < cfg.max_steps:
        ...
        for micro_step in range(cfg.grad_accum_steps):
            ...
            t_compute = time.time()
            attn_ctx = sdpa_kernel(SDPBackend.FLASH_ATTENTION) if attn_backend == "sdpa" else nullcontext()
            with sync_ctx, amp_ctx, fp8_ctx_factory(), attn_ctx:
                _, loss = model(x, y)
