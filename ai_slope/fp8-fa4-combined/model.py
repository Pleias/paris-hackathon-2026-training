...
def _linear(in_f, out_f, bias=False):
    """Use TE Linear when available so FP8 kernels actually fire."""
    if _TE_AVAILABLE:
        return te.Linear(in_f, out_f, bias=bias)
...
class CausalSelfAttention(nn.Module):
    def __init__(self, n_embd, n_head, seq_len, dropout):
        ...
        self.n_head = n_head
        self.n_embd = n_embd
        self.c_attn  = _linear(n_embd, 3 * n_embd)
        self.c_proj  = _linear(n_embd, n_embd)
...
class MLP(nn.Module):
    def __init__(self, n_embd, dropout):
        super().__init__()
        self.fc   = _linear(n_embd, 4 * n_embd)
        self.proj = _linear(4 * n_embd, n_embd)
...
class GPT(nn.Module):
    def __init__(self, vocab_size, seq_len, n_layer, n_head, n_embd, dropout=0.0):
        ...
        self.lm_head = _linear(n_embd, vocab_size)
